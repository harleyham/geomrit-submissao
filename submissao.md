...
- **Envio de e-mail para grupo por evento** (card "Enviar e-mail para grupo" na página de participantes, `POST /admin/events/:id/email-groups`): restrito ao superadministrador e administradores do evento (staff não). Grupos de destinatários (com contagem exibida no select): **todos os inscritos do evento** (inscrições aprovadas), **por papel do evento** (`event_user_roles`, inscrito ou não — Participante, Revisor, Palestrante, Professor, Apresentador Oral/Pôster, Administrador, Staff) e **participantes de uma atividade** (inscritos na atividade, incluindo tipos de convívio). Destinatários aptos: conta ativa (`is_public=1`), aprovada e com e-mail (mesmo critério do lembrete automático de evento). Uma mensagem por destinatário (template `direct-message`) com a **identidade do evento** (remetente, assinatura e logo), confirmação prévia com a contagem ("Enviará para N destinatários") e `confirm` do navegador. Respeita o master switch global e o switch de e-mails do evento: com envio desativado as mensagens são registradas como *suppressed* (visíveis na "Lista de e-mails suprimidos" do dashboard) e nada é entregue até reativação; mensagens enfileiradas podem ser canceladas em lote por "Limpar fila". Conteúdo em texto simples com assinatura automática do evento.
...
- Cadastro e edição de atividades com **datas e horários opcionais** (sem `required` no form; sem data e/ou horários, a atividade tem o rótulo unificado **A definir**, badge âmbar na listagem administrativa e é ordenada por último). Nesse estado a **sala é removida** em vez de bloquear o save (`resolveRoomAllocation`/`syncTargetAssignments`: `room_assignments` exige data e horários NOT NULL) — aviso "Sala removida: defina data e horários para realocar." (ou "Sala não alocada..." na criação). Atividades A definir não entram na grade/barras públicas e não têm janela de check-in QR; a chamada administrativa continua disponível. Campo opcional de link da transmissão de vídeo (máximo de 500 caracteres; vazio remove o link), exibido na listagem administrativa (ação "Vídeo") e no card público "Atividades do Evento". A atividade também pode configurar **número máximo de participantes** (`max_participants`, vazio = sem limite — aplicado às auto-inscrições de participantes; o admin pode exceder com destaque visual "Vagas esgotadas" na listagem), **aprovação obrigatória** (`requires_approval`: inscrições de participantes viram pedidos aguardando análise) e **automática para participantes** (`required_for_participants` = obrigatória / `default_for_participants` = padrão para todos — inscritos em todos os participantes, retroativo ao marcar, fora da escolha na inscrição) — ver "Atividades automáticas para participantes" e "Inscrição do participante em minicursos pela página do evento".
...
- Página de salas com formulário de criação/edição (nome, tipo e capacidade sempre visível), listagem (Nome, Tipo, Capacidade, Alocações, Ações), card "Aguardando sala" (etapas e atividades sem etapas ainda sem sala, com botão "Alocar sala" para abrir o formulário de edição), reserva de sala pelo próprio evento (quando não há atividades) e botões de relatório "Ocupação por dia" e "Agenda por sala".
...
- Quarta visualização **Grade (dia × hora) – Barras**: eixo temporal contínuo por dia (dias nas colunas, dinamico da primeira à última hora ocupada; 56px por hora com linhas-guia por hora), com **barras retangulares da cor do tipo de atividade** cobrindo do `time_start` ao `time_end` da atividade/etapa (etapas herdam a cor e o rótulo `Atividade : Etapa`; atividade sem etapas multi-dia repete a barra em todos os dias do intervalo); atividades simultâneas dividem a coluna do dia em faixas iguais (quantas atividades no mesmo período, tantas barras lado a lado, por atribuição greedy de "faixas" — a barra e colocada na primeira faixa livre que ja terminou). Regras: sem `time_end` a barra cobre 1 hora (cruzando meia-noite, é posicionada no dia seguinte); etapa com data mas sem horário rende **barra do dia inteiro** (tooltip "horário a definir" via `title` com o nome completo); estados vazios idênticos às demais visões e rolagem horizontal em telas estreitas.
...
- A formação acadêmica possui a opção especial `Não possui curso de graduação`, disponível em todas as áreas de formação; quando selecionada, os campos Titulação e Status são gravados como nulos e ficam bloqueados (desabilitados, sem valor) nos formulários voltados ao usuário (`/login/complete-profile` e `/author/profile`; nos formulários administrativos continuam ocultos).
...
- A administração pode criar, editar e remover inscrições manualmente. Toda inscrição manual possui conta vinculada: o admin seleciona uma conta ativa existente ou cria uma conta nova — no mesmo padrão da importação de listas (CSV/XLSX), a senha é aleatória e invisível ao admin, e o participante recebe e-mail com link de uso único (72h) para definir a própria senha; uma inscrição com artigo submetido não pode ser removida diretamente.
...
- **Atividades automáticas para participantes — obrigatórias (`event_activities.required_for_participants`) e padrão (`event_activities.default_for_participants`)**: configuradas no formulário de criação/edição da atividade (checkboxes "Obrigatória para participantes" e "Padrão para todos os participantes", disponíveis apenas quando o papel `participant` é elegível e o tipo não é logístico — extras nunca podem ser automáticas). As duas flags têm a **mesma mecânica** e diferem apenas pelo rótulo/badge ("Obrigatória" em destaque primário; "Padrão" na cor de sucesso): todos os participantes do evento são inscritos automaticamente e a atividade **não aparece como escolha na inscrição** (fica fora de `getPublicEventActivities`; a página de inscrição exibe aviso de que as atividades obrigatórias/padrão já estão incluídas). A união forçada é feita no servidor (`enforceRequiredActivities` na inscrição e re-seleção pública; `enforceRequiredActivitiesAdmin` nos fluxos administrativos), e a validação aceita os ids das automáticas em POSTs de inscrições existentes, então mesmo um POST adulterado mantém a inscrição. **Inscrição retroativa**: ao criar/editar uma atividade com qualquer flag ligada, `applyAutomaticActivityEnrollments` inscreve na hora todos os participantes **aprovados** do evento ainda não inscritos (transação; minicurso respeita vagas — inscreve até esgotar e o redirect informa quantos ficaram de fora); desligar a flag **não remove** inscrições existentes. **Importação por evento** (`/admin/events/:id/import-users`): cada pessoa criada/atualizada e inscrita no evento já entra nas atividades automáticas (sem checagem de vagas, padrão dos fluxos administrativos). Em eventos com inscrição sujeita à análise, a automática entra na `requested_activity_ids` do pedido e é **aprovada automaticamente** na análise (no formulário de análise aparece marcada, travada, sempre incluída e excluída da mensagem "aprovadas todas"). Em `/evento/:id/atividades` a automática inscrita aparece com badge e sem checkbox habilitado (input oculto quando editável); o endpoint de auto-inscrição de minicurso recusa a desmarcação. Badges "Obrigatória"/"Padrão" na listagem administrativa de atividades, na página pública (Cards/Lista), no formulário do participante (marcadas e travadas) e na análise de pedidos. As auditorias existentes continuam válidas; presença e certificados não mudam.
...
- A chamada de uma atividade lista como participantes apenas as pessoas inscritas nela por `participant_activity_enrollments`; palestrantes, professores, revisores e apresentadores também podem aparecer por seus papéis próprios no evento, **desde que compatíveis com os papéis elegíveis da atividade** — e a **lista de presença impressa** (`attendance-print`, com ou sem `session_id`) aplica o mesmo filtro da chamada: só entram na lista as pessoas cujos papéis no evento/atribuições cruzam com os papéis elegíveis da atividade (evitando que holders de outros papéis, ex. staff não inscrito, apareçam na lista impressa).
...
- Tanto a aprovação de solicitação pública quanto a criação direta de usuário pelo administrador geram mensagem com **link de uso único para o usuário definir a própria senha** (72h, hash em `user_setup_tokens`; cancelamento do e-mail revoga o token). A criação direta (`/admin/users/new`) envia o mesmo e-mail da importação global (`imported-account`, "Sua conta foi criada"). Nenhuma senha é enviada por e-mail.
- Criação direta de usuário em `/admin/users/new` exige **apenas nome e e-mail**: a conta nasce no padrão de importação (senha inutilizável — hash aleatório desconhecido, `password_changed=0`, `profile_completed=0`, aprovada e ativa), e o e-mail guia o usuário a definir a senha e completar o perfil no primeiro login; envio de senha no formulário é rejeitado. Com o master global desligado, a mensagem de sucesso informa que o e-mail não foi enfileirado. Demais dados pessoais do formulário permanecem opcionais.
...
- Certificado próprio por atividade: atividades marcadas com "Certificado próprio" (ex.: minicursos) emitem certificado isolado com a carga horária da própria atividade e presença mínima configurável por atividade (0–100%, padrão 75%); as horas dessas atividades são excluídas da consolidação de todos os certificados por papel; o visual é herdado da regra do papel Participante, e a seção "Certificados por atividade (próprios)" na página de Certificados oferece emissão individual, em lote por atividade e no botão global "Emitir todos"; a verificação pública exibe o nome da atividade.
...
- Fase 0: ao selecionar a opção `Não possui curso de graduação`, os campos Titulação e Status são gravados como nulos nas rotas de criação/edição; em `views/complete-profile.ejs` e `views/public/participant-profile.ejs` (Área do Participante) os campos ficam visíveis e bloqueados (`disabled`, valores limpos, `required` desligado — JS `syncTitulacaoVisibility`), e nos formulários administrativos (`views/admin/users/form.ejs`, `views/admin/events/participant-form.ejs`) continuam ocultos. O express-validator de `/login/complete-profile` (`security/validation.js`) aceita Titulação/Status vazios (`optional({ values: 'falsy' })`), validando o `isIn` apenas quando preenchidos.
