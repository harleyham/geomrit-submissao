...
3. Informe nome, tipo, intervalo/data, **hora de início e hora de término**, carga horária e se a atividade emite certificado. Datas e horários são **opcionais**: deixe-os em branco para a atividade ficar com o rótulo **A definir** (útil enquanto o cronograma ainda não está fechado); a listagem administrativa mostra o badge âmbar **A definir** e a data "A definir" nas demais telas. Sem data e horários completos a atividade **não pode manter sala alocada** (ao salvar com sala pré-selecionada, a alocação é removida e o aviso "Sala removida: defina data e horários para realocar." é exibido — repreencha data e horários e salve de novo para realocar). Atividades **A definir** não aparecem na grade/barras públicas e não têm janela de check-in por QR (a presença administrativa na chamada continua disponível). Para **Palestra** e **Minicurso**, preencha também uma descrição breve ou ementa, com até 2000 caracteres. **Carga horária**: se informada (> 0), é a carga total da atividade e **prevalece sobre as etapas** — as etapas ficam sem carga própria (zeradas e bloqueadas na edição); se deixada vazia, a carga da atividade é a **soma das cargas das etapas**.
...
6. (Opcional) Selecione a **sala** da atividade — disponível quando a atividade não possui etapas e somente com data e horários completos (veja a Seção 9); sem data/horários, salvar com sala selecionada remove a alocação em vez de bloquear o save.
...
O participante escolhe atividades durante a inscrição e pode reconfigurá-las na própria página de inscrição (`/evento/:id/inscricao`); o administrador também pode fazer essa associação. **As atividades extras (café da manhã, coffee break, brunch, almoço e jantar) não aparecem como opções de escolha do participante** — na inscrição pública e no formulário administrativo do participante elas são omitidas (a presença nelas é registrada normalmente na chamada). **As atividades automáticas (obrigatórias e padrão) também não aparecem como escolha**: entram sozinhas na inscrição de todos, com aviso na página de inscrição; em `/evento/:id/atividades` aparecem como inscritas, com badge "Obrigatória"/"Padrão" e checkbox bloqueado (a inclusão é reforçada no servidor em todas as rotas de escolha). A página `/evento/:id/atividades` lista **somente as atividades em que a pessoa está inscrita** (com as avaliações de cada uma), mais a seção "Atividades de meu interesse". Em eventos com inscrição sujeita à análise, a seleção aprovada pela organização não pode ser alterada pelo participante.
...
- pedidos de subsídio;
- **pedidos de inclusão em atividades** (incluindo inscrições em minicurso aguardando a análise da organização, e os pedidos de minicurso marcados pelo checkbox na página do evento que ainda não foram analisados: cada linha leva à edição do participante, onde os botões **"Sim (aprovar)" / "Não (negar)"** registram a decisão);
...
A seção "Backup e Restauração" no dashboard concentra as operações de manutenção do banco de dados, acessíveis **exclusivamente** ao `admin@admin.com` (superadministrador):
...
A importação de usuários em **Usuários** (`/admin/users/import`) envia automaticamente os e-mails de criação de conta: como o admin que importa já aprova os acessos, os novos usuários recebem imediatamente o link individual de uso único para definir senha (válido por 72 horas), sem clique adicional. Senhas nunca são enviadas por e-mail. A autorização só é manual quando o master switch global está desativado — nesse caso, a página de resultado exibe um aviso e os e-mails ficam pendentes até a reativação e a autorização pelo admin. Já a importação de inscrições em eventos (`/admin/events/:id/import-users`) continua exigindo **Autorizar e-mails aos participantes** na página de resultado.
