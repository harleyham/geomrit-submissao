...
- A normalização foi substituída por sanitização restrita a espaços nas extremidades e conversão para minúsculas, preservando pontos e identificadores `+` em login, cadastro, submissão, usuários, participantes, inscrição em evento e perfil.
- Registros já persistidos não são alterados automaticamente, pois não é possível inferir a posição original dos pontos.
