...
- **Gestão de eventos**: **qualquer usuário autenticado pode criar eventos** — o criador recebe automaticamente o papel de administrador do seu evento e pode delegar papéis (inclusive admin) a pessoas inscritas no evento; a página "Meus Eventos" (`/admin/events`) lista apenas os eventos atribuídos ao usuário (o superadmin `admin@admin.com` vê todos); **alterar a Data de Início do evento propaga o deslocamento em dias para todas as atividades, etapas, salas e janelas do cronograma que não foram editadas manualmente no mesmo save**; cronograma público, janelas de inscrição/submissão/certificados e administração restrita aos eventos atribuídos; a página inicial lista os publicados e, em bloco próprio **"Eventos Encerrados"** (badge âmbar "Encerrado"), os eventos encerrados — que permanecem acessíveis para consulta e certificados
 - **Atividades no evento público**: a seção "Atividades do Evento" da página pública `/evento/:id` oferece três visualizações alternáveis — **Cards** (padrão, agrupados por tipo, no mesmo formato do painel administrativo), **Lista** (com data, horário e sala) e **Grade (dia × hora)** (dias nas colunas, hora de início nas linhas, e cada célula mostra o intervalo **início–fim** e o nome; uma atividade sem etapas com intervalo de vários dias aparece em **todos os dias**); **Grade (dia × hora) – Barras** posiciona cada atividade/etapa como barra retangular da cor do tipo sobre um eixo de horários por dia (eixo dinâmico, 56px/hora; atividades simultâneas dividem o dia em faixas iguais lado a lado; sem término = 1h; etapa sem horário = barra do dia inteiro); cada atividade mostra descrição/ementa, data e horário, e a **sala** (quando não tem etapas); quando tem etapas, cada uma é listada com data, horário e sala; o botão **Etapas** (só aparece quando há etapas) abre a página pública somente-leitura `/evento/:id/atividades/:id/etapas`, e o link "Assistir transmissão" abre o vídeo em nova aba; abaixo da programação, a seção **Transmissões** lista as atividades e etapas com vídeo (link próprio ou herdado da atividade; aviso "Transmissão prevista" quando ainda sem link)
...
- **Importação de participantes**: importação administrativa de planilhas CSV ou XLSX, com auto-detecção de delimitador (vírgula ou ponto-e-vírgula), compatibilidade com quebras de linha Windows (CRLF) e Unix (LF), detecção flexível de colunas, criação/atualização de contas, relatório pessoa por pessoa com status (Sucesso/Falha/Ignorado) e download em CSV; via evento (`/admin/events/:id/import-users`) também inscreve os participantes automaticamente (disponível ao administrador do evento); via usuários (`/admin/users/import`) cria apenas contas sem inscrição (exclusiva do superadmin)
- **Submissão de artigos**: formulário público, rascunhos, múltiplos revisores, parecer individual, deliberação final administrativa; submissões pendentes **editáveis pelo autor** até a designação de revisor e enquanto o prazo de submissões estiver aberto (botão desabilitado com a justificativa quando bloqueado; rascunhos editáveis a qualquer tempo)
- **Área do Participante (`/author`)**: inscrições, rascunhos e submissões; administradores acessam com a própria conta pelo menu superior das telas admin; consultas públicas por código com padrão Post/Redirect/Get, para não reenviar formulário ao voltar no navegador
- **Revisão**: painel do revisor (`/reviewer`) com artigos pendentes, envio de parecer e recomendação; o papel **Revisor** é atribuído por evento (página Papéis) e dá acesso ao painel; a atribuição de revisores a artigos considera apenas os detentores do papel no evento
- **Participantes e papéis**: conta única por pessoa, com papéis **exclusivamente por evento** (administrador, staff, revisor, palestrante, professor e apresentador) — não existem mais papéis globais: a atribuição é feita **somente** na página Papéis (`/admin/events/:id/roles`) ou na edição do participante do evento (sincronizadas), para **inscritos no evento**, podendo acumular vários papéis (salvar substitui o conjunto); a atribuição não altera o cadastro da pessoa, e os papéis do superadministrador (`admin@admin.com`) são imutáveis; a área de usuários (`/admin/users`) é exclusiva do superadmin; telefone internacional e formação acadêmica (área, curso, titulação, status) editáveis no cadastro de participantes e usuários
- **Prévia "Área do Participante" (visualização como o usuário)**: o superadmin abre a área de um usuário em `/admin/users/:id/participant` e navega o sistema como ele (inscrições e ações registradas em seu nome); enquanto a prévia está ativa, **todas as páginas** exibem tarja laranja no topo com nome, e-mail e o link "Sair da visualização" (qualquer acesso a `/admin/*` também encerra a prévia e restaura a sessão do admin, já com a identidade correta na página de aterrissagem)
- **Papel Staff**: designação **exclusivamente por evento** (página de Papéis ou edição do participante do evento); opera os seus eventos — gestão de participantes, presença (manual, em lote e por QR) com impressão de listas, QR de check-in e crachás, edição de atividades e etapas existentes, certificados, artigos/revisões e relatórios do evento — sem poder criar/apagar evento, apagar usuário, criar/apagar atividades/etapas, gerir papéis/salas ou acessar outros eventos e módulos administrativos
...
- **Reset de senha (admin)**: o botão "Resetar Senha" na listagem de usuários (exclusiva do superadmin) gera senha temporária (troca obrigatória no primeiro acesso) e envia ao usuário **e-mail com link de uso único** (72h) para definir a nova senha, sem senha no corpo do e-mail; com o envio desativado ou conta sem e-mail, a senha temporária é exibida ao administrador em página do painel para comunicação por canal seguro. Demais usuários trocam a própria senha na Área do Participante (botão **Alterar Senha**)
- **Criação de usuário sem senha**: em `/admin/users/new` (exclusiva do superadmin) o obrigatório é apenas **nome e e-mail**; a conta nasce aprovada e sem senha (padrão de importação) e o usuário recebe **e-mail com link de uso único (72h)** para definir a própria senha e completar os dados pessoais no primeiro acesso
- **Cadastro sem senha (definição por link)**: o formulário público "Solicitar Cadastro" não pede senha — o **e-mail deve ser repetido no campo "Confirme E-mail"**; o envio só é aceito quando os dois valores conferem (validação no navegador e no servidor), e após o envio o usuário é encaminhado para a página de confirmação (/cadastro/sucesso), onde é avisado a verificar inclusive a caixa de Lixo Eletrônico ou Spam. A conta é criada com senha interna inutilizável e, após a aprovação administrativa, o usuário recebe **e-mail com link de uso único (72h)** para definir a própria senha; nenhuma senha transita por e-mail. Quem perder o link usa "Esqueci a senha" na `/login`. Aplica-se também ao e-mail de aprovação do superadministrador ao criar usuários em `/admin/users`.
- **Esqueci a senha (pública)**: link "Esqueci a senha" abaixo de "Solicitar novo cadastro" na `/login` abre o formulário `/login/esqueci-senha`; ao informar o e-mail cadastrado, a pessoa recebe e-mail com **link de uso único de 72h** para `/definir-senha`, onde define a nova senha. A senha atual só é substituída quando o link é efetivamente usado (não usá-lo não reseta nem bloqueia nada); resposta idêntica para e-mail existente ou inexistente contra enumeração de contas, e aviso quando o envio de e-mails está desativado
...
- **Presença por QR Code**: botão "QR Presença" na listagem de atividades imprime uma folha letter por etapa (ou da atividade, quando sem etapas) com evento, atividade, data, etapa, **sala** e QR Code (idem na lista de assinaturas); o usuário escaneia o código e marca a própria presença **com tolerância de 10 minutos antes e depois** do período da etapa/atividade (janela por data+hora em UTC-3), no papel que exerce **dentre os papéis elegíveis da atividade** (cruzados com os papéis da pessoa no evento; a lista exibida no check-in e a gravação do backend respeitam a configuração da atividade), exigindo login e vínculo à atividade para participantes. Alternativamente, cada participante possui um QR Code pessoal (crachá) com código estável por evento em `/evento/:id/qr-presenca` (exibível e imprimível); na chamada, o administrador lê o QR pela câmera (jsQR servido localmente) ou digita o código para marcar a presença da pessoa no papel correspondente
...
- **Atividades de interesse**: o participante inscrito e aprovado marca, nos cards de "Atividades do Evento" em `/evento/:id`, as atividades que deseja assistir (todos os tipos **exceto minicursos**, que exigem inscrição, e atividades extras: café da manhã, coffee break, brunch, almoço e jantar); a escolha é **gravada automaticamente a cada clique** (`POST /evento/:id/interesses` via fetch, com botão "Salvar interesses" como fallback para navegador sem JavaScript) e aparece na seção "Atividades de meu interesse" de `/evento/:id/atividades`, sem interferir na inscrição, presença ou certificados
- **Atividades automáticas (obrigatórias e padrão para todos)**: cada atividade pode ser marcada como **"Obrigatória para participantes"** (badge "Obrigatória") ou **"Padrão para todos os participantes"** (badge "Padrão") — em ambos os casos todos os participantes do evento são inscritos automaticamente (retroativo ao marcar; desmarcar não remove inscrições; minicurso respeita vagas) e a atividade **não aparece como escolha na inscrição** (a página de inscrição informa que elas já estão incluídas). A inclusão vale para inscrições públicas, administrativas, importação por evento e aprovação de pedidos (aprovada automaticamente na análise); POSTs adulterados não removem a inscrição
- **Horários e salas**: atividades e etapas têm hora de início e término (datas e horários são **opcionais** — em branco a atividade fica **A definir**, sem sala até novo prazo); o administrador cadastra quantas salas quiser por evento, classificadas por tipo (Tipo 1, Tipo 2, Tipo 3, Auditório, Mini Auditório, Foyer, Coffee break, Restaurante e Posters) com capacidade livre informada e nomeadas; a sala é designada por etapa, pela atividade sem etapas ou pelo evento (quando ainda não há atividades, reservando-a em todos os dias do intervalo), com bloqueio automático de sobreposição de horário na mesma sala e verificação transacional; nos formulários de atividade/etapa o seletor de sala traz apenas as salas livres no dia/horário escolhidos, e erros de gravação preservam os dados já digitados
...
- **Certificado próprio por atividade**: atividades marcadas como "Certificado próprio" (ex.: minicursos) emitem um certificado isolado com a carga horária da própria atividade, com presença mínima configurável por atividade (0–100%, padrão 75%); as horas dessas atividades são **excluídas da consolidação de todos os certificados por papel** e não são cobradas duas vezes; o visual do certificado (fundo, cor, título e texto) é herdado da regra do papel **Participante**; emissão individual, em lote por atividade e no botão global "Emitir todos"; a consulta pública exibe o nome da atividade
- **Verificação pública**: consulta pública da autenticidade de certificados pelo código de verificação
- **Carga horária**: prioridade atividade > etapas > 0 — se a atividade tem carga horária definida (> 0), ela é a carga total (e as etapas ficam sem carga própria, zeradas e bloqueadas); sem carga na atividade, a carga é a soma das horas das etapas; certificados de atividades qualificadas valem a carga total efetiva da atividade (não apenas das etapas frequentadas)
- **E-mails transacionais**: fila persistente via SMTP/Zoho com master switch global e por evento (ambos desligados por padrão), confirmação de cadastro/aprovação, recebimento e resultado da análise de inscrição, resultado da análise de pedidos de inclusão em atividade (aprovado/negado), alteração administrativa de atividades, lembrete no dia anterior, certificado emitido/reemitido, alterações de transmissão e autorização explícita após importações; mensagens geradas com envio desativado ficam **supensas** e aparecem no dashboard do superadmin com lista consultável e possibilidade de exclusão definitiva
...
| Super-admin | Acesso a reset/backup/usuários e aos cards "Paleta", "Envio global de e-mails" e "Backup e Restauração" do dashboard, só com conta ativa, aprovada, senha trocada e `is_admin` confirmado no banco (não apenas pelo e-mail na sessão); `admin@admin.com` vê e administra todos os eventos do sistema e seus papéis são imutáveis (ninguém atribui/altera/remove papéis dessa conta); os demais administradores são escopados aos seus eventos, não têm acesso a `/admin/users` e veem o dashboard personalizado ("Meu painel") apenas com os dados dos eventos que administram |
| Papéis por evento | Autoridade exclusivamente em `event_user_roles`: papéis (inclusive revisor e staff) podem ser atribuídos a qualquer conta ativa e aprovada, inscrita ou não no evento (não inscrito não gera inscrição); sessões não são promovidas a admin (identidade global derivada do banco a cada request; `is_admin` é só o marcador da linha semente do superadmin); staff tem acesso ao "Meu painel" (`/admin/dashboard`) escopado aos eventos em que é staff, sem os módulos de artigos/revisão |
