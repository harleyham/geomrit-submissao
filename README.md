...
- **E-mails transacionais**: fila persistente via SMTP/Zoho com master switch global e por evento (ambos desligados por padrão), confirmação de cadastro/aprovação, recebimento e resultado da análise de inscrição, alteração administrativa de atividades, lembrete no dia anterior, certificado emitido/reemitido, alterações de transmissão e autorização explícita após importações
...
| CSRF | Token por sessão, validação timing-safe, aceito por header/field/cookie, cobertura de uploads multipart, 403 em requisições inválidas |
