...
| Papéis por evento | Autoridade exclusivamente em `event_user_roles`: papéis (inclusive revisor e staff) só são atribuídos a inscritos no evento; sessões não são promovidas a admin (identidade global derivada do banco a cada request; `is_admin` é só o marcador da linha semente do superadmin) |
