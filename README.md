...
- **Participantes e papéis**: conta única por pessoa, com papéis **exclusivamente por evento** (administrador, staff, participante, revisor, palestrante, professor e apresentador) — não existem mais papéis globais: a atribuição na página Papéis não altera o cadastro da pessoa, e só pode ser feita para **inscritos no evento** (o superadmin pode atribuir a qualquer conta); a área de usuários (`/admin/users`) é exclusiva do superadmin; telefone internacional e formação acadêmica (área, curso, titulação, status) editáveis no cadastro de participantes e usuários
...
| Super-admin | Acesso a reset/backup/dashboard/usuários só com conta ativa, aprovada, senha trocada e `is_admin` confirmado no banco (não apenas pelo e-mail na sessão); `admin@admin.com` vê e administra todos os eventos do sistema; os demais administradores são escopados aos seus eventos e não têm acesso a `/admin/users` nem a `/admin/dashboard` |
