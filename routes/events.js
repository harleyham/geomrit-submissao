...
function validateEventDates({
  ...
  if (has_article_submission) {
    ...
    if (submission_end && review_end && review_end < submission_end) {
      return 'A data final do período de análise não pode ser anterior ao fim do período de submissões.';
    }
  }
...
router.post('/', requireSignedUser, strictLimiter, runEventAssetUpload, (req, res, next) => {
  ...
  // O criador já é admin do evento via event_user_roles (acima); a sessão não
  // é promovida: administração global é exclusiva do superadmin.
    ...
    for (const row of rows) {
      ...
      if (existing) {
        ...
        if (!existingReg) {
          ...
          alreadyRegistered += 1;
          report.push({ name: existing.name, email: canonicalEmail, status: 'success', detail: 'Usuário existente — já inscrito no evento' });
        }
        skipped += 1;
      } else {
        try {
