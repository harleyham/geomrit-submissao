...
- Verificação: boot local; E2E com usuários de teste (aprovados, evento 2) confirmando "enrolled"/"cheio"/liberação de vaga/"requested"/cancelamento de pedido e o fluxo SIM/NÃO (negar → card "Pedido negado pela organização" travado + endpoint recusando novo pedido; novo pedido → botões reaparecem; aprovar → matrícula criada e contador "Inscritos: N/M" incrementado); dados de teste removidos e configuração do evento 2 restaurada. Docs (`submissao.md`, `manual.md`) atualizadas. `Status: implementado e validado em desenvolvimento (aguardando commit)`.
...
- Verificação: `node --check` e EJS OK; **E2E 13/13** no dev (contas temporárias via bcrypt + fetch com CSRF/cookies): login comum → `GET /new` 200 → criação 302 com evento gravado + papel admin do criador + sessão promovida; lista própria mostra o evento; atribuição de Palestrante a B (302 success) com papel gravado e `is_speaker` ligada sozinha; 403 em evento alheio e no `/edit` de quem só é speaker; lista de B sem o evento; dados de teste apagados e verificados. Sem mudança de banco. `Status: implementado e validado em desenvolvimento (aguardando commit)`.
...
- Verificação: EJS OK; **E2E 7/7** com servidor forçado em `TZ=UTC` (pior caso): home e `/admin/events` mostrem `01/12/2026 – 31/12/2026` para o evento 5 e **nenhum** `30/11`/`30/12` (login do superadmin com senha temporária; hash original restaurado e conferido contra o backup do dia). Sem mudança de banco. `Status: implementado e validado em desenvolvimento (aguardando commit)`.
...
- Verificação: EJS OK; **E2E 6/6** no dev (senha temporária só durante o teste, hash restaurado): mensagem própria presente, `form action` de reset na linha do admin, sem `js-delete-user` na linha própria, demais linhas mantêm Excluir. Sem mudança de rota/banco. `Status: implementado e validado em desenvolvimento (aguardando commit)`.
