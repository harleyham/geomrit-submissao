...
- Renderização: eixo vertical dinâmico (`floor`/`ceil` da primeira/última hora ocupada, padrão 07:00–22:00 quando só há barras de dia inteiro), 67.2px por hora (+20% sobre os 56px iniciais, a pedido do usuário, para acomodar mais horários no dia) com linhas-guia por hora e rótulo de hora à esquerda; coluna por dia (min-width 140px, `flex` + rolagem horizontal no `grid-wrap`); por dia, atribuição **greedy de faixas** (a barra entra na primeira faixa livre cujo fim já passou) determina largura vertical: cada barra recebe `left`/`width` em `%` do nº de faixas simultâneas (N simultâneas → N barras lado a lado); barra com tooltip `title` (horário + nome completo), compactação (`padding`/fonte menor) quando altura < 40px, mínimo de 16px.
