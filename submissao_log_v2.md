...
> **Sobre a V0.34**: versão em desenvolvimento desde 08/09/2026. Consolidado até agora: formação acadêmica com "Não possui curso de graduação" — Titulação e Status ficam **visíveis e bloqueados** (`disabled`, valores limpos) em `/login/complete-profile` e `/author/profile` em vez de ocultos (formulários administrativos mantêm ocultação), com o `express-validator` de `/login/complete-profile` aceitando os campos vazios (`optional({ values: 'falsy' })`, `isIn` apenas quando preenchidos); erro de validação do `express-validator` em `/login/complete-profile` **re-exibe o formulário com os dados digitados** (antes caía na tela genérica 400 sem o formulário, perdendo tudo). Mater separado do ciclo de versão: corte da V0.33 em master (master acompanha a versão).

> **Sobre a V0.33**: versão em desenvolvimento desde 28/08/2026 (continuação do ciclo da V0.32), consolidando por período: **02–05/09/2026** — temas de paleta global renomeados e documentados (Azul, Verde, Claro, Alto contraste), token `--surface-2` aplicado a todos os cards/views, botões padronizados na página de salas ("Alocar sala", "Editar", "Remover") com impressão/PDF das agendas corrigida (CSP), card "Identidade dos e-mails" sem fundo azul, banner de e-mails em `/admin/events` com cor por estado e links "Início" no menu superior do dashboard e de eventos; opcionalidade de voltar evento publicado a **Rascunho** (`POST /admin/events/:id/unpublish`) com botão "Voltar para Rascunho"; **migração idempotente** reforçada (passos de colunas novas, ex. `system_settings.theme`, rodam em todos os boots mesmo com `user_version` já na versão final — correção de 500 no login em banco legado); cadastro público sem senha (`/cadastro` cria conta bloqueada até aprovação, com definição de senha por link de uso único no e-mail de aprovação) e confirmação de e-mail; **atividades obrigatórias para participantes** (flag com reforço no servidor em todos os pontos de escolha) e badge "Obrigatória"; **quarta visão pública "Barras"** (grade dia × hora com barras por horário, divisão por faixas em clusters de sobreposição, +20% na altura/hora e cor por tipo); cor por tipo de atividade nas visões públicas e regra de **carga horária (atividade > etapas)** com bloqueio/badge nas listas; esqueci a senha público e notificação de decisão de pedido de atividade; validação de atividade (revisor elegível, carga vazia) e reexibição do formulário com draft; datas de análise configuráveis (pode se sobrepor ao fim das submissões); mojibake corrigido em `routes/`. Branch de release: `V0.33` (master acompanha a versão).
...
- Decisões do usuário (levantamento): UI **na página de participantes de cada evento** (`/admin/events/:id/participants`); permissão **superadmin + administrador do evento** (staff não); grupos **todos os inscritos do evento**, **por papel do evento** e **participantes de uma atividade**; comportamento confirmado — uma mensagem por destinatário, prévia de contagem + `confirm`, mesmo template do envio individual, respeito ao master switch (enfileirada ou suprimida) e controle pela fila existente.
- `services/email.js`: nova `queueGroupDirectEmail({ event, recipients, subject, body })` — para cada destinatário, `enqueueEmail` com `eventId` do evento, template `direct-message` (`messageType: 'direct'`), payload com `recipientName` (nome da pessoa, e não o e-mail), `body` e a **identidade do evento** (`getEventIdentity`: remetente, assinatura e logo); `groupKey`=`direct-group:<event.id>:<uuid>` única por chamada e `dedupeKey` por destinatário; retorna `{ queued, suppressed }` (suppressed = inserida com envio bloqueado por master global/evento, com motivo em `last_error`). Exportada.
- `routes/events.js`:
  - helpers: `GROUP_EMAIL_REGISTRATION_WHERE` (inscrição aprovada — `COALESCE(registration_status,'approved')='approved'` em `event_registrations` —, conta ativa `is_public=1`, `approval_status='approved'`, e-mail não vazio; mesmo critério do lembrete automático de evento), `getGroupEmailRecipients(eventId, group, roleId, activityId)` (3 queries: `event_registrations JOIN users`; papel via `event_user_roles` — inscrito ou não; atividade via `participant_activity_enrollments → event_registrations → users`) e `getGroupEmailOptions(eventId)` (contagem por grupo: `allCount`, `roles` com rótulo de `CERTIFICATE_ROLES`/`EVENT_ROLE_LABELS`, `activities` do evento).
  - `GET /:id/participants`: para admins do evento, carrega `groupEmail` (opções + contagens + `emailPermission = canQueueEmail(event.id)`); staff não recebe (card oculto).
  - `POST /:id/email-groups` (`requireEventAdminOnly`, `strictLimiter`): aceita o select combinado do formulário (valores `all`, `role:<papel>`, `activity:<id>`); valida grupo/papel/atividade pertencentes ao evento, assunto e mensagem; erro amigável nos casos: campos vazios, papel inválido, atividade inexistente no evento, grupo sem destinatários; envia via `queueGroupDirectEmail` e redireciona para a página de participantes com mensagem de sucesso contando destinatários/enfileirados/suprimidos e observação quando o envio está desativado.
  - `GET /:id/email-groups/options`: endpoint JSON auxiliar com opções+contagens+estado do envio do evento (para uso futuro/refresh).
- `views/admin/events/participants.ejs`: novo card **"Enviar e-mail para grupo"** no topo da página (`#email-group-card`), somente para admins; select "Destinatários" com três grupos (`<optgroup>` por papel e por atividade) e contagens exibidas em cada opção (`data-count`); alerta âmbar/erro adicional quando `canQueueEmail` está bloqueado; tamanho assuntos (200) e mensagem, textarea (texto simples; assinatura automática do evento); contador ao vivo "Enviará para N destinatário(s)" (script com `cspNonce`) e `confirm('Enviar e-mail para N destinatário(s)?')` no submit.
- Comportamento de fila: mensagens usam a mesma fila do sistema — com master global ou envio do evento desativados, cada mensagem é gravada com `status='suppressed'` (motivo em `last_error`) e nada é entregue; com o envio ativo, vão para `queued` e podem ser canceladas em lote pelo "Limpar fila" existente no dashboard. Grupo `role:staff` no teste devolveu 0 enfileiradas + 1 suprimida com master desligado; após reativar mensagens passam a enfileirar normalmente (`canQueueEmail` consultado a cada chamada).
- Verificação E2E (sandbox isolada na porta 3124 + cópia do banco): login admin → página de participantes renderiza com card/contador/form e contagem "Todos" (24); POST "todos" → 24 e-mails enfileirados (`event_id=2`, `template direct-message`, `recipient_name` = nome da pessoa, `group_key` único por chamada, `status='queued'`); POST por atividade ("Fundamentos de Python para Supercomputação e IA", 18 inscritos) → 18/18 com mesma query de contagem; POST por papel (alvo: `admin`, 2 pessoas) → 2/2; POST com papel inválido → "Papel inválido"; POST sem assunto/mensagem → "Preencha o assunto e a mensagem"; envio com master global desativado → 0 enfileiradas + 1 suprimida com o motivo correto; login como staff → 403 no POST (404 na página via redirect ao dashboard) e card oculto (302/redirect como esperado).
- Docs: `submissao.md` (novo bullet de recursos sobre envio de e-mail em grupo) e `manual.md` (seção 6 — nova subseção "Envio de e-mail para grupo" com os três grupos, contador, mensagem individual, critério de destinatários e o funcionamento da fila).
- Verificação E2E (sandbox isolada na porta 3124 + cópia do banco): login admin → página de participantes renderiza com card/contador/form e contagem "Todos" (24); POST "todos" → 24 e-mails enfileirados (`event_id=2`, template `direct-message`, `recipient_name` = nome da pessoa, `group_key` único por chamada, `status='queued'`); POST por atividade ("Fundamentos de Python para Supercomputação e IA", 18 inscritos) → 18/18 com a mesma query de contagem; POST por papel (alvo: `admin`, 2 pessoas) → 2/2; POST com papel inválido → "Papel inválido"; POST sem assunto/mensagem → "Preencha o assunto e a mensagem"; envio com master global desativado → 0 enfileiradas + 1 suprimida com o motivo correto; login como staff → card oculto e 403 no POST e no endpoint de opções.
...
- `views/admin/users/form.ejs`: no modo criação o campo **Senha foi removido** (o campo continua disponível na edição, com o mesmo "deixe vazio para manter a atual"); **Nome** passa a ser obrigatório (`required`); nota explicativa sobre o e-mail com link de definição.
- `security/validation.js`: contrato `userForm` agora exige `name` (`trim().notEmpty()`), aplica-se às rotas de criação e de edição (POST/PUT `/admin/users/:id`) que compartilham o mesmo contrato; `password` deixou de ser aceito (a senha nunca é definida pelo admin na criação).
- `routes/users.js` (POST `/admin/users`): valida apenas nome + e-mail (CPF validado somente se preenchido) e rejeita envio de `password` ("Informe apenas nome e e-mail: a senha é definida pelo usuário via link enviado por e-mail."); a conta passa a ser criada no **padrão de importação** — `bcrypt` de `crypto.randomBytes(32)` (senha inutilizável), `password_changed=0`, `profile_completed=0`, `is_public=1`, `approval_status='approved'`, flags `is_*` zeradas; demais campos opcionais (país, instituição, telefone, documentos, formação, áreas de revisor) continuam gravados.
- E-mail: trocado `queueAccountApproved` por `queueImportedAccount` (template `imported-account`, "Sua conta foi criada", mesmo texto da importação global), com token de uso único de 72h em `user_setup_tokens` e `dedupeKey: 'imported-account:<id>:<timestamp>'`; antes de enfileirar consulta `canQueueEmail(null)` — com o master switch global desligado, a mensagem de sucesso avisa que o e-mail não foi enfileirado, e com envio a mensagem cita o endereço destinatário.
- Pós-link (fluxo existente, sem alteração): `/definir-senha` grava a senha e `password_changed=1`; no primeiro login o `profile_completed=0` redireciona para `/login/complete-profile`.
- Sem migração de banco. Conta criada sem senha **não consegue logar** até usar o link (mesmo comportamento das contas importadas).
- Verificação: `node --check` em `routes/users.js`, `security/validation.js` e `services/email.js`; renderização EJS do formulário nos modos criação/edição (campo senha ausente só na criação); smoke HTTP em sandbox isolada (porta 3115, DB novo): POST `/admin/users` com só nome+e-mail criou conta com `password_changed=0`/`profile_completed=0`, e-mail `imported_account` na fila `email_outbox` com link 72h; uso do link definiu a senha (reuso rejeitado com "inválido, expirou ou já foi utilizado"); login redirecionou para `/login/complete-profile`; duplicidade por e-mail e nome vazio rejeitados com as mensagens do formulário; master switch desligado → mensagem específica sem enfileirar.
- Status: **concluído (código + verificação técnica)**. Os campos de dados pessoais do formulário de criação permanecem disponíveis como preenchimento administrativo opcional.
...
### Quarta visão da página pública: Grade (dia × hora) – Barras

- Requisito do usuário: nova visualização na seção "Atividades do Evento" de `/evento/:id`, semelhante à Grade (dia × hora), mas com **barras retangulares** (cor de fundo do tipo de atividade, texto na cor do tipo) cobrindo do `time_start` ao `time_end`, e atividades no mesmo período exibidas como barras lado a lado.
- Implementação apenas em `views/public/event.ejs` (sem rota/banco): radio `activities-view-bars` + label "Grade (dia × hora) – Barras" na `activities-toggle` e regras `:checked` equivalentes; cálculo das ocorrências (`barOcc`): minutos desde 00:00 por atividade/etapa, fim ausente = +60 min (cruzando meia-noite a ocorrência muda para o dia seguinte), etapa com data e sem horário = **barra do dia inteiro** (`fullDay`), atividade sem etapas multi-dia repete a barra em cada dia (limite 92 dias, como na grade), etapas herdam o tipo da atividade pai e rótulo `Atividade : Etapa`.
- Renderização: eixo vertical dinâmico (`floor`/`ceil` da primeira/última hora ocupada, padrão 07:00–22:00 quando só há barras de dia inteiro), 67.2px por hora (+20% sobre os 56px iniciais, a pedido do usuário, para acomodar mais horários no dia) com linhas-guia por hora e rótulo de hora à esquerda; coluna por dia (min-width 140px, `flex` + rolagem horizontal no `grid-wrap`); por dia, a divisão de largura passou a ser **por clusters de sobreposição** (correção: antes o nº de faixas era o máximo do dia inteiro e uma atividade isolada à tarde ficava com a largura das simultâneas da manhã — idem com sobreposição parcial, ex.: 8–10 e 9–11:30): itens não sobrepostos abrem cluster próprio (1 faixa = largura total) e dentro de cada cluster vale o greedy de faixas (N simultâneas → N barras lado a lado); barra com tooltip `title` (horário + nome completo), compactação (`padding`/fonte menor) quando altura < 40px, mínimo de 16px.
- Validação (render EJS com mock): 8/8 — toggle/rotulo, 3 simultâneas em 3 faixas, sem fim → 1h (`12:00–13:00`), etapa sem hora → "Dia inteiro", multi-dia repetido, etapa nomeada, badge "Obrigatória", grade antiga intacta.
- `Status: implementado e validado localmente; efetivo após reinício do servidor.`
...
- Regra nova (definida em 02/09): a carga horária efetiva de uma atividade é a carga **definida na atividade (> 0)**; se vazia, a **soma das cargas das etapas**; senão 0. Se a atividade define a carga, os campos de carga das etapas são **zerados e bloqueados para edição** (persistência em `syncSessionWorkloadLock` no salvar da atividade, força 0 no salvar da etapa via `activityWorkloadLocked`, input desabilitado no formulário de etapas com aviso da carga da atividade, e migração idempotente de boot em `db-reset.js` zerando horas de etapas de atividades com carga > 0).
- Certificados (`getRoleActivityAttendance`): atividade **qualificada** pelo percentual de presença vale a carga **total efetiva** da atividade (não mais a soma das etapas frequentadas); elegibilidade por percentual/tipo inalterada; `total_workload_hours` contínua somando apenas atividades qualificadas.
- Exibição da carga efetiva (carga da atividade > 0 senão soma das etapas) nas listas públicas (`getPublicEventActivities`, `getInterestActivities` e query do evento `/evento/:id` com subquery `sessions_workload`), lista de inscrição em atividade, form e análise de participante e listagem administrativa de atividades (selo "(etapas)" só quando a carga vem das etapas).
- Extras na mesma leva: validador de atividade aceita o papel `reviewer` (marcar todos os papéis não falha mais) e carga horária vazia é aceita (`optional({values:'falsy'})`); erro de validação do formulário de atividade re-renderiza a própria página com o rascunho (antes, tela genérica de erro com único link "Voltar ao início"); ordenação cronológica corrigida em atividades (data + hora, não só data + nome) e etapas (dia + hora antes de `sequence_no`), em listas admin/públicas; checkbox "Contabilizar esta atividade para certificados" agora reflete o valor gravado no modo edição; relatório de datas do evento na spec (`submissao.md`): análise pode começar durante o período de submissões e `review_end` não pode ser anterior ao fim de `submission_end`.
- Valores do banco real migrados (evento 3): zeradas as cargas de 10 etapas de 4 atividades com carga definida (Seminário de abertura 1h, Seminário principal 4h, Minicurso Pós-graduação #2 12h); Minicurso Pós-graduação #1 (carga por etapas) preservado; ordenação cronológica verificada por SQL no evento 3.
- `Status: implementado e validado localmente; efetivo após reinício do servidor.`
...
### Ajustes de UI: botões na página de salas, impressão/PDF das agendas, nomes das paletas

- Página de salas (`/admin/events/:id/rooms`): no card "Aguardando sala", os links "Alocar sala" (atividades sem etapas e etapas) viraram botões (`btn btn-secondary`, largura mínima 96 px e `white-space:nowrap` para o texto em linha única); em "Salas cadastradas", o link "Editar" também virou botão `btn btn-secondary` e o "Remover" passou a `btn btn-danger` (estilo uniforme do sistema), com `nowrap` na célula de ações.
- Impressão/PDF das agendas de salas (`/admin/events/:id/rooms/agenda` e `/rooms/occupancy`): o botão "Imprimir / PDF" não fazia nada — causa: `onclick="window.print()"` inline com nonce no atributo, mas navegadores não aceitam nonce em handlers de evento (a CSP `script-src-attr` só libera handler inline com `'unsafe-inline'`/hash, mantidas fechadas por hardening). Correção: botão com classe `.print-button` e `addEventListener` em `<script nonce>`, padrão já usado em `certificates.ejs`.
- Nomes das paletas ("Paleta do sistema" no dashboard, `services/theme.js`): `ligem` "LIGEM (padrão)" → **Azul**; `sipam` "SIPAM / Censipam" → **Verde**; mantidos "Claro" e "Alto Contraste". Apenas rótulos — IDs (`ligem`, `sipam`, …) e preferência salva inalterados. Fallbacks em `dashboard.ejs` atualizados.
